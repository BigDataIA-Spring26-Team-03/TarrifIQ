"""
ITA-specific Federal Register ingestion pipeline:
1. Fetch & store raw ITA docs -> S3 + Snowflake (DOWNLOADED)
2. Parse documents -> sections + hash (PARSED)
3. Extract HTS codes -> ITA_NOTICE_HTS_CODES (HTS_EXTRACTED)
4. Chunk semantically -> ITA_CHUNKS (CHUNKED)
...
def _ita_pipeline_max_docs() -> int | None:
    raw = os.environ.get("ITA_PIPELINE_MAX_DOCS", "").strip()
    if raw == "" or raw == "0":
        return None
    try:
        n = int(raw)
...
def _ita_sql_row_limit(cap: int | None) -> int:
    if cap is None:
...
def _parallel_shard_count() -> int:
    raw = os.environ.get("ITA_PIPELINE_PARALLEL_SHARDS", str(_DEFAULT_PARALLEL_SHARDS)).strip()
    try:
        n = int(raw)
    except ValueError:
...
def _task_fetch_and_store_raw() -> None:
    from ingestion.ita_client import fetch_and_load_ita_incrementally

    cap = _ita_pipeline_max_docs()
    loaded = fetch_and_load_ita_incrementally(
        test_mode=False,
        cutoff_year=2020,
        batch_size=50,
        max_documents=cap,
    )
    logger.info("ITA fetch complete loaded=%d", loaded)
...
def _task_parse_documents(shard_id: int = 0, total_shards: int = 1) -> None:
    ...
    from ingestion.ita_client import S3_BUCKET
    ...
    try:
        row_limit = _ita_sql_row_limit(_ita_pipeline_max_docs())
        cur.execute(
            f"""
        ...
        if not rows:
            return
        if not S3_BUCKET:
            logger.error("ITA parse failed: S3_BUCKET not set")
            return
        ...
        for document_number, document_type, title, agency_names_raw, publication_date, s3_key in rows:
            try:
                ...
                cur.execute(
                    """
                    UPDATE ITA_FEDERAL_REGISTER_NOTICES
        ...
        conn.commit()
        logger.info("ITA parse complete shard=%s/%s parsed=%d", shard_id, total_shards, parsed_count)
    finally:
        cur.close()
        conn.close()
...
def _task_extract_hts_codes(shard_id: int = 0, total_shards: int = 1) -> None:
    ...
    from ingestion.ita_client import S3_BUCKET
    from ingestion.federal_register_client import _get_s3_client, _parse_xml_to_text
    from ingestion.ita_hts_extractor import run_extraction_pipeline
    ...
    try:
        ...
        for document_number, title, agency_names_raw, raw_json, s3_key in docs:
            try:
                ...
                conn.commit()
        logger.info("ITA extract complete shard=%s/%s extracted=%d failed=%d", shard_id, total_shards, extracted, failed)
    finally:
        cur.close()
        conn.close()
...
def _task_chunk_documents(shard_id: int = 0, total_shards: int = 1) -> None:
    ...
    try:
        ...
        if not rows:
            return
        if not S3_BUCKET:
            logger.error("ITA chunk failed: S3_BUCKET not set")
            return
        ...
        for document_number, document_type, title, agency_names_raw, publication_date, content_hash, s3_key in rows:
            try:
                ...
                for chunk in chunks:
                    ...
                    cur.execute(
                        """
                        INSERT INTO ITA_CHUNKS (chunk_id, document_number, chunk_index, chunk_text, section, word_count)
                        VALUES (%s, %s, %s, %s, %s, %s)
                        """,
                        (
                            chunk_id,
        ...
        conn.commit()
        logger.info("ITA chunk complete shard=%s/%s chunked=%d", shard_id, total_shards, chunked)
    finally:
        cur.close()
        conn.close()
