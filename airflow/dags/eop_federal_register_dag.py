"""
EOP (Executive Office of the President) Federal Register ingestion pipeline:
1. Fetch & store raw EOP docs -> S3 + Snowflake (DOWNLOADED)
2. Parse documents -> sections + hash (PARSED)
3. Extract HTS codes -> NOTICE_HTS_CODES_EOP (HTS_EXTRACTED)
4. Chunk semantically -> EOP_CHUNKS (CHUNKED)

Agency: executive-office-of-the-president
Document types: Presidential Document, Notice
...
def _eop_pipeline_max_docs() -> int | None:
    raw = os.environ.get("EOP_PIPELINE_MAX_DOCS", "").strip()
    if raw == "" or raw == "0":
        return None
    try:
        n = int(raw)
...
def _eop_sql_row_limit(cap: int | None) -> int:
    if cap is None:
...
def _parallel_shard_count() -> int:
    raw = os.environ.get("EOP_PIPELINE_PARALLEL_SHARDS", str(_DEFAULT_PARALLEL_SHARDS)).strip()
    try:
        n = int(raw)
    except ValueError:
...
def _task_fetch_and_store_raw() -> None:
    from ingestion.eop_client import fetch_and_load_eop_incrementally

    cap = _eop_pipeline_max_docs()
    loaded = fetch_and_load_eop_incrementally(
        test_mode=False,
        cutoff_year=2021,
        batch_size=50,
        max_documents=cap,
    )
    logger.info("EOP fetch complete loaded=%d", loaded)
...
def _task_parse_documents(shard_id: int = 0, total_shards: int = 1) -> None:
    ...
    from ingestion.eop_client import S3_BUCKET
    ...
    try:
        row_limit = _eop_sql_row_limit(_eop_pipeline_max_docs())
        cur.execute(
            f"""
            ...
            WHERE (processing_status IS NULL OR processing_status = 'pending' OR processing_status = 'downloaded')
              AND YEAR(publication_date) >= 2021
        ...
        if not rows:
            return
        if not S3_BUCKET:
            logger.error("EOP parse failed: S3_BUCKET not set")
            return
        ...
        conn.commit()
        logger.info("EOP parse complete shard=%s/%s parsed=%d", shard_id, total_shards, parsed_count)
    finally:
        cur.close()
        conn.close()
...
def _task_extract_hts_codes(shard_id: int = 0, total_shards: int = 1) -> None:
    ...
    from ingestion.eop_client import S3_BUCKET
    from ingestion.federal_register_client import _get_s3_client, _parse_xml_to_text
    from ingestion.eop_hts_extractor import run_extraction_pipeline
    ...
    try:
        ...
        for document_number, title, agency_names_raw, raw_json, s3_key in docs:
            try:
                ...
                conn.commit()
        logger.info("EOP extract complete shard=%s/%s extracted=%d failed=%d", shard_id, total_shards, extracted, failed)
    finally:
        cur.close()
        conn.close()
...
def _task_chunk_documents(shard_id: int = 0, total_shards: int = 1) -> None:
    ...
    try:
        ...
        if not rows:
            return
        if not S3_BUCKET:
            logger.error("EOP chunk failed: S3_BUCKET not set")
            return
        ...
        for document_number, document_type, title, agency_names_raw, publication_date, content_hash, s3_key in rows:
            try:
                ...
                for chunk in chunks:
                    ...
                    cur.execute(
                        """
                        INSERT INTO EOP_CHUNKS (chunk_id, document_number, chunk_index, chunk_text, section, word_count)
                        VALUES (%s, %s, %s, %s, %s, %s)
                        """,
                        (
                            chunk_id,
        ...
        conn.commit()
        logger.info("EOP chunk complete shard=%s/%s chunked=%d", shard_id, total_shards, chunked)
    finally:
        cur.close()
        conn.close()
...
with DAG(
    dag_id="eop_federal_register_ingest",
    default_args=DEFAULT_ARGS,
    description="EOP Federal Register ingestion: fetch -> parse -> hts -> chunk",
    schedule_interval="@daily",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["tariffiq", "ingestion", "eop", "presidential"],
) as dag:
    shard_count = _parallel_shard_count()

    task_fetch = PythonOperator(
        task_id="eop_fetch_and_store_raw",
        python_callable=_task_fetch_and_store_raw,
    )

    for shard_id in range(shard_count):
        parse_t = PythonOperator(
            task_id=f"eop_parse_documents_shard_{shard_id}",
            python_callable=_task_parse_documents,
            op_kwargs={"shard_id": shard_id, "total_shards": shard_count},
        )
        extract_t = PythonOperator(
            task_id=f"eop_extract_hts_codes_shard_{shard_id}",
            python_callable=_task_extract_hts_codes,
            op_kwargs={"shard_id": shard_id, "total_shards": shard_count},
        )
        chunk_t = PythonOperator(
            task_id=f"eop_chunk_documents_shard_{shard_id}",
