...
Policy and Trade agents run sequentially (can be parallelized later).
"""
...
def hitl_node(state: TariffState) -> Dict[str, Any]:
    """
    HITL gate — logs the escalation and returns state unchanged.
    In production: write to HITL_RECORDS and notify via Slack/email.
    """
    logger.warning(
        "hitl_escalation query=%s reason=%s",
        state.get("query"),
...
def after_classification(state: TariffState) -> str:
    """Route to HITL if confidence < 0.80, otherwise continue to rate."""
...
def after_synthesis(state: TariffState) -> str:
    """Route to HITL if citation validation failed."""
...
def run_pipeline(query: str) -> Dict[str, Any]:
    """
    Run the full TariffIQ pipeline for a user query.

    Args:
        query: Plain English user query

    Returns:
        Final TariffState as dict
    """
