"""
ITA Federal Register ingestion for TariffIQ.

Duplicates the ITC Federal Register ingestion flow with ITA-specific filters
and ITA-specific Snowflake table targets.

Agency: international-trade-administration
Table:  ITA_FEDERAL_REGISTER_NOTICES
S3:     raw/ita/{year}/{month}/{document_number}.xml
"""
...
ALL_AGENCIES = ["international-trade-administration"]
TEST_AGENCIES = ["international-trade-administration"]
TARIFF_SEARCH_TERMS = ["tariff rate quota", "duty rate", "HTSUS"]
ALLOWED_DOC_TYPES = {"Notice", "Rule", "Proposed Rule"}

SNOWFLAKE_BATCH_SIZE = 50
DEFAULT_S3_BUCKET = "tariff-iq-federal-registry-bucket"
S3_ITA_RAW_PREFIX = "raw/ita"
S3_BUCKET = (os.environ.get("S3_BUCKET") or DEFAULT_S3_BUCKET).strip() or None


def upload_ita_raw_xml_to_s3(document_number: str, xml_content: bytes, publication_date: str) -> Optional[str]:
    ...
    try:
        ...
        s3_key = f"{S3_ITA_RAW_PREFIX}/{year_str}/{month_str}/{document_number}.xml"
        ...
        logger.warning("Failed to upload ITA XML doc=%s error=%s", document_number, exc)
        return None
...
def _fetch_full_text(xml_url: str, document_number: str = None, publication_date: str = None) -> tuple[str, Optional[str]]:
    ...
    try:
        ...
        if document_number and publication_date:
            s3_key = upload_ita_raw_xml_to_s3(document_number, xml_content, publication_date)
...
def fetch_and_load_ita_incrementally(
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT document_number FROM ITA_FEDERAL_REGISTER_NOTICES")
        existing_doc_numbers = {row[0] for row in (cur.fetchall() or []) if row and row[0]}
        cur.close()
        conn.close()
    except Exception as exc:
        logger.warning("Could not load existing ITA document numbers: %s", exc)
                ...
                for page_results in page_iter:
                    for item in page_results:
                        ...
                        if len(batch) >= batch_size:
                            total_loaded += load_ita_to_snowflake(batch)
                            batch = []
...
def load_ita_to_snowflake(rows: List[dict]) -> int:
    if not rows:
        return 0
    ...
    try:
        for i in range(0, len(rows), SNOWFLAKE_BATCH_SIZE):
            ...
            merge_sql = f"""
            MERGE INTO ITA_FEDERAL_REGISTER_NOTICES AS t
            USING (
                SELECT
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest ITA Federal Register documents")
    parser.add_argument("--test", action="store_true", help="Test mode: fetch 1 page from ITA")
    args = parser.parse_args()
    loaded = fetch_and_load_ita_incrementally(test_mode=args.test, cutoff_year=2020)
    print(f"Loaded {loaded} documents into ITA_FEDERAL_REGISTER_NOTICES")
