"""
ITC Federal Register ingestion for TariffIQ.

Duplicates the USTR Federal Register ingestion flow with ITC-specific filters
and ITC-specific Snowflake table targets.
"""
...
ALL_AGENCIES = ["international-trade-commission"]
TEST_AGENCIES = ["international-trade-commission"]
...
S3_ITC_RAW_PREFIX = "raw/itc"
S3_BUCKET = (os.environ.get("S3_BUCKET") or DEFAULT_S3_BUCKET).strip() or None


def upload_itc_raw_xml_to_s3(document_number: str, xml_content: bytes, publication_date: str) -> Optional[str]:
    ...
    try:
        ...
        s3_key = f"{S3_ITC_RAW_PREFIX}/{year_str}/{month_str}/{document_number}.xml"
        ...
        logger.warning("Failed to upload ITC XML doc=%s error=%s", document_number, exc)
        return None
...
def _fetch_full_text(xml_url: str, document_number: str = None, publication_date: str = None) -> tuple[str, Optional[str]]:
    ...
    try:
        ...
        if document_number and publication_date:
            s3_key = upload_itc_raw_xml_to_s3(document_number, xml_content, publication_date)
...
def fetch_and_load_itc_incrementally(
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT document_number FROM ITC_DOCUMENTS")
        existing_doc_numbers = {row[0] for row in (cur.fetchall() or []) if row and row[0]}
        cur.close()
        conn.close()
    except Exception as exc:
        logger.warning("Could not load existing ITC document numbers: %s", exc)
                ...
                for page_results in page_iter:
                    for item in page_results:
                        ...
                        if len(batch) >= batch_size:
                            total_loaded += load_itc_to_snowflake(batch)
                            batch = []
...
def load_itc_to_snowflake(rows: List[dict]) -> int:
    if not rows:
        return 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest ITC Federal Register documents")
    parser.add_argument("--test", action="store_true", help="Test mode: fetch 1 page from ITC")
    args = parser.parse_args()
    loaded = fetch_and_load_itc_incrementally(test_mode=args.test, cutoff_year=2018)
    print(f"Loaded {loaded} documents into ITC_DOCUMENTS")
